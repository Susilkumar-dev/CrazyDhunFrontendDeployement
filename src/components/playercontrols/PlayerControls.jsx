...
import {
  FaPlayCircle, FaPauseCircle, FaStepBackward, FaStepForward,
  FaVolumeUp, FaVolumeDown, FaVolumeMute,
  FaHeart, FaRegHeart, FaPlus, FaEllipsisH
} from 'react-icons/fa';
import { PlayerContext } from '../../context/PlayerContext';
...
const PlayerControls = ({ onPlayerOpen }) => {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 bg-gradient-to-r from-purple-900/90 via-gray-900/90 to-cyan-900/90 backdrop-blur-xl border-t border-purple-500/30 shadow-[0_-4px_25px_rgba(0,0,0,0.5)] px-4 py-2 z-50">
      <div className="flex items-center justify-between">
        ...
        {/* Playlist Add Modal */}
        {isPlaylistModalOpen && (
          <div className="absolute bottom-24 right-4 bg-gray-800/95 backdrop-blur-xl border border-purple-500/30 rounded-xl shadow-2xl p-3 w-64 animate-fade-in">
            <div className="flex justify-between items-center mb-2 px-2">
              <h4 className="text-white text-sm font-semibold">Add to Playlist</h4>
              <button 
              ...
              {userPlaylists.length > 0 ? (
                userPlaylists.map(playlist => (
                  <button
                    key={playlist._id}
                    onClick={() => { addSongToPlaylist(playlist._id, currentSong._id); setIsPlaylistModalOpen(false); }}
                    className="w-full text-left p-3 text-gray-300 hover:bg-purple-700/30 hover:text-white rounded-lg transition-all duration-200 flex items-center"
                  >
                    <div className="w-8 h-8 bg-gradient-to-br from-purple-600 to-cyan-500 rounded-md mr-3 flex items-center justify-center">
                      <FaEllipsisH className="text-white text-xs" />
                    </div>
                    <span className="truncate">{playlist.name}</span>
                  </button>
                ))
              ) : (
                <p className="p-3 text-gray-400 text-sm text-center">No playlists found</p>
              )}
            </div>
          </div>
        )}
        ...
        {/* Left Side: Album Info */}
        <div className="w-1/4 flex items-center min-w-0">
          <button onClick={onPlayerOpen} className="flex items-center group space-x-3 text-left">
            <div className="relative">
              <img src={buildImageUrl(currentSong.coverArtPath)} alt="Album Art"
                className="w-14 h-14 rounded-xl shadow-[0_0_20px_rgba(110,80,230,0.6)] object-cover transition-transform duration-300 group-hover:scale-105" />
              <div className="absolute inset-0 bg-gradient-to-b from-transparent to-black/50 rounded-xl opacity-0 group-hover:opacity-100 transition-opacity"></div>
            </div>
            <div className="hidden sm:block max-w-xs">
              <h4 className="font-semibold text-white truncate group-hover:text-cyan-300 transition-colors duration-300">{currentSong.title}</h4>
        ...
        <div className="w-2/4 flex flex-col items-center px-2">
          <div className="flex items-center space-x-5 mb-2">
            <button onClick={playPreviousSong} disabled={!hasPrev}
              className="p-2 text-gray-300 hover:text-cyan-400 disabled:opacity-30 disabled:cursor-not-allowed transition-all duration-300 hover:scale-110">
              <FaStepBackward size={18} />
            </button>
            
            <button onClick={togglePlayPause} className="p-2 text-white hover:scale-110 transition-transform duration-300 transform">
              {isPlaying ? 
                <FaPauseCircle size={44} className="drop-shadow-[0_0_10px_rgba(110,230,230,0.7)]" /> : 
                <FaPlayCircle size={44} className="drop-shadow-[0_0_10px_rgba(110,230,230,0.7)]" />
              }
            </button>
          ...
          <div className="flex items-center w-full max-w-xl gap-3">
            ...
            <input
              type="range" min="0" max={duration || 0} value={currentTime}
              onChange={(e) => seek(e.target.value)}
              className="w-full h-1.5 bg-gray-700/60 rounded-full appearance-none cursor-pointer [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:h-3 [&::-webkit-slider-thumb]:w-3 [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:bg-gradient-to-r [&::-webkit-slider-thumb]:from-purple-500 [&::-webkit-slider-thumb]:to-cyan-400 [&::-webkit-slider-thumb]:border-0 [&::-webkit-slider-thumb]:cursor-grab"
            />
        ...
        <div className="w-1/4 flex items-center justify-end space-x-3">
          <button onClick={handleLikeToggle}
            className={`p-2 transition-all duration-300 ${isLiked ? 'text-pink-500 hover:text-pink-400' : 'text-gray-400 hover:text-pink-500'} hover:scale-110`}>
            {isLiked ? <FaHeart size={20} /> : <FaRegHeart size={20} />}
          </button>
          
          <div className="relative group" onMouseEnter={() => setShowVolumeSlider(true)} onMouseLeave={() => setShowVolumeSlider(false)}>
            <button onClick={toggleMute} className="p-2 text-gray-400 hover:text-cyan-400 transition-all duration-300 hover:scale-110">
              <VolumeIcon />
            </button>
            
            <div className={`absolute bottom-full right-0 mb-3 p-2 bg-gray-800/90 backdrop-blur-md rounded-lg shadow-lg transition-opacity duration-300 ${showVolumeSlider ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
              <input
                type="range" min="0" max="1" step="0.01"
                value={isMuted ? 0 : volume}
                onChange={(e) => changeVolume(e.target.value)}
                className="h-24 bg-gray-700/60 rounded-full appearance-none [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:h-3 [&::-webkit-slider-thumb]:w-3 [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:bg-gradient-to-r [&::-webkit-slider-thumb]:from-purple-500 [&::-webkit-slider-thumb]:to-cyan-400 [&::-webkit-slider-thumb]:border-0 [&::-webkit-slider-thumb]:cursor-grab"
                orient="vertical"
              />
            </div>
          </div>
          ...
          <input
            ...
            value={isMuted ? 0 : volume}
            onChange={(e) => changeVolume(e.target.value)}
            className="hidden md:inline-block w-24 h-1.5 bg-gray-700/60 rounded-full appearance-none [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:h-3 [&::-webkit-slider-thumb]:w-3 [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:bg-gradient-to-r [&::-webkit-slider-thumb]:from-purple-500 [&::-webkit-slider-thumb]:to-cyan-400 [&::-webkit-slider-thumb]:border-0 [&::-webkit-slider-thumb]:cursor-grab"
          />
        </div>
      </div>
    </div>
