                ...
                <AnimatePresence mode="wait">
                    {activeTab === 'songs' && (
                        ...
                        >
                            ...
                            <div className={`rounded-2xl overflow-hidden ${isDarkMode ? 'bg-gray-800/50' : 'bg-white/50'} backdrop-blur-md border ${isDarkMode ? 'border-gray-700' : 'border-gray-200'} shadow-lg`}>
                                <div className={`grid grid-cols-12 px-6 py-4 text-sm font-medium ${isDarkMode ? 'text-gray-400' : 'text-gray-500'} hidden md:grid`}> {/* Hidden on small screens */}
                                    <div className="col-span-1">#</div>
                                    <div className="col-span-6">Title</div>
                                    <div className="col-span-3">Album</div>
                                    <div className="col-span-2 flex justify-end">
                                    ...
                                    {artistSongs.map((song, index) => {
                                        ...
                                        return (
                                            <motion.div
                                                ...
                                                animate={{ opacity: 1, y: 0 }}
                                                transition={{ delay: index * 0.05 }}
                                                className={`flex items-center px-4 py-3 md:grid md:grid-cols-12 md:px-6 md:py-4 group hover:${isDarkMode ? 'bg-gray-700/50' : 'bg-gray-100/50'} transition-colors cursor-pointer ${
                                                    isCurrentSong ? (isDarkMode ? 'bg-purple-900/30' : 'bg-purple-100/50') : ''
                                                }`}
                                                onClick={() => handleSongClick(song)}
                                            >
                                                {/* Mobile Play Button & Index */}
                                                <div className="flex-none w-10 md:col-span-1 flex items-center justify-center relative">
                                                    <span className="text-gray-400 absolute group-hover:opacity-0 md:group-hover:flex md:group-hover:relative">
                                                        {index + 1}
                                                    </span>
                                                    <button
                                                        onClick={(e) => {
                                                        ...
                                                        className={`text-white p-2 rounded-full bg-green-500 hover:bg-green-400 transition-colors absolute opacity-0 group-hover:opacity-100 md:relative md:opacity-100 ${
                                                            isSongPlaying ? 'opacity-100' : ''
                                                        }`}
                                                    >
                                                        {isSongPlaying ? <FaPause size={10} /> : <FaPlay size={10} />}
                                                    </button>
                                                ...
                                                <div className="flex-1 min-w-0 flex items-center md:col-span-6">
                                                    <img
                                                        src={buildImageUrl(song.coverArtPath)}
                                                        alt={song.title}
                                                        className="w-12 h-12 object-cover rounded-lg mr-4 shadow-md flex-none"
                                                    />
                                                ...
                                                <div className="hidden md:block md:col-span-3 text-sm opacity-70 truncate">
                                                    {song.album || 'Single'}
                                                </div>
                                                
                                                <div className="flex-none flex justify-end items-center space-x-2 md:space-x-4 md:col-span-2 ml-auto">
                                                    <button
                                                        onClick={(e) => { 
                                                            e.stopPropagation(); 
                                                            toggleLike(song._id); 
                            ...
                            {Object.entries(albums).map(([albumName, albumData]) => {
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <p 
                                            className="font-semibold truncate hover:text-green-500 transition-colors text-sm sm:text-base"
                                            onClick={() => navigate(`/dashboard/album/${encodeURIComponent(albumName)}`)}
                                        >
                                            {albumName}
                                        </p>
                                        <p className="text-xs sm:text-sm opacity-70">{albumData.songs.length} songs</p>
                                    </motion.div>
                                );
                            })}
                        </motion.div>
